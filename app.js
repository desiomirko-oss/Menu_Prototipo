const VERSION = "12.1-BG-IMAGE-FIX";
...
function applyConfig() {
    ...
    if (logoUrl) document.getElementById('logo-container').innerHTML = `<img src="${escapeHTML(logoUrl)}" id="app-logo" style="max-height:${escapeHTML(getVal('Logo_Height', '80px'))}; object-fit:contain;" translate="no" class="notranslate">`;
...
function updateLayout() {
    setTimeout(() => {
        const h = document.getElementById('main-header'), sh = document.getElementById('sub-header'), m = document.getElementById('main-content');
        if (h && m) {
            let t = h.offsetHeight;
            if (sh && sh.style.display === 'flex') t += sh.offsetHeight;
            m.style.paddingTop = `calc(${t}px + 20px)`;
        }
    }, 50);
}
...
async function fetchMenu() {
    ...
    try {
        ...
        for(let i=1; i<rows.length; i++){
            ...
            if(c.length >= 3 && c[0]) fullData.push({ _id: i, macro: c[0], cat: c[1], name: c[2], desc: c[3], price: c[5], gf: c[6], vegan: c[7], veg: c[8], noalc: c[9], active: c[10]||'TRUE', photo: c[11], ar: c[12], details: c[13] || '' });
        }
...
function renderLevel2(m) {
    ...
    cats.forEach(c => {
        ...
        if (imgUrl) inner = layout === 'grid' ? `<div class="cat-img-wrapper"><img src="${escapeHTML(imgUrl)}" class="cat-img-grid" loading="lazy"></div>` + inner : inner + `<div class="cat-img-wrapper"><img src="${escapeHTML(imgUrl)}" class="cat-img-list" loading="lazy"></div>`;
...
function toggleFilter(f) { activeFilters = activeFilters.includes(f) ? [] : [f]; renderLevel3(currentMacro, currentCat, true); }

function renderLevel3(m, c, isF = false) {
    currentMacro = m; currentCat = c;
    if (!isF) activeFilters = []; 
    ...
    items.forEach(i => {
        ...
        const hasD = i.details.trim() !== '';
        const badgeRow = `<div class="badge-container"><div class="badge-group">${b}</div>${hasD?'<span class="inline-chevron">›</span>':''}</div>`;
        ...
        container.innerHTML += `<div class="menu-card ${hasD?'clickable-card':''}" ${hasD?`onclick="openItemDetails(${i._id})"`:''}><div class="item-card"><div style="flex-grow:1;"><div class="item-name notranslate">${escapeHTML(i.name)}</div><div class="item-desc">${escapeHTML(i.desc)}</div><div class="item-price notranslate">${escapeHTML(i.price)}</div></div>${i.photo ? `<img src="${escapeHTML(i.photo)}" class="item-photo" style="margin-left:10px;" loading="lazy">` : ''}</div>${badgeRow}${ar}</div>`;
    });
    if(!isF && navigationStack[navigationStack.length-1] !== 'page-items') navigationStack.push('page-items');
    showPage('page-items');
}
...
function openItemDetails(id) {
    ...
    container.innerHTML = `<div class="details-page-card">${item.photo ? `<img src="${escapeHTML(item.photo)}" class="detail-photo">` : ''}<div style="padding: 0 20px;"><div class="badge-container" style="justify-content:center; margin-bottom:15px;"><div class="badge-group">${b}</div></div><div class="detail-title notranslate">${escapeHTML(item.name)}</div><div class="detail-price notranslate">${escapeHTML(item.price)}</div><div class="detail-desc">${escapeHTML(item.desc)}</div><div class="detail-long-text">${escapeHTML(item.details).replace(/\n/g, '<br>')}</div>${item.ar ? `<div style="width:100%; display:flex; justify-content:center; margin-top:20px;"><a href="${escapeHTML(item.ar)}" target="_blank" class="ar-btn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path></svg> Vedi Piatto in AR</a></div>` : ''}</div></div>`;
...
function showPage(p) {
    ...
    if (b) p === 'page-macro' ? b.classList.remove('active') : b.classList.add('active');
    const sh = document.getElementById('sub-header');
    if (sh) sh.style.display = (p === 'page-items') ? 'flex' : 'none';
